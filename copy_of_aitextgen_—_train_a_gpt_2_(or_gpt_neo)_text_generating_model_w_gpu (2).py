...
In this step, I am checking which GPU is currently available in my Google Colab session.  
Colab usually provides GPUs like Nvidia P4, T4, P100, or V100. All of these are fine for fine-tuning GPT-2 (124M), but T4 and P100 are better for faster text generation because they have more VRAM.  

If I get a T4 or V100, I can enable `fp16=True` during training to make it faster and more memory-efficient.  

Let’s run the code below to check which GPU I got.
"""

!nvidia-smi

"""**Output Observation:**  
The active GPU is **Tesla T4**, which is suitable for both training and text generation.  
I can also enable `fp16=True` during training for faster and more memory-efficient performance.
